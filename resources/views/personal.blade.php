@extends('welcome')
@section('content')

		
			
        <div class="grid grid-cols-1 gap-6 mt-6   sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-1">
		
		
		          <div class="px-1 py-1">
               <pre>   
		            <p class="text-gray-700"><b>Name 		   		:</b> Protus Mukhongo Kakai</p>
					<p class="text-gray-700"><b>Date of Birth 		:</b> 4th January 1971</p>
					<p class="text-gray-700"><b>Nationality   		: </b>Kenyan</p>
					<p class="text-gray-700"><b>Marital Status 		:</b> Married</p>
					<p class="text-gray-700"><b>Languages Spoken 	:</b> English and Kiswahili</p>
                 </pre>  
				   </div>	
           

		 </div>  
@endsection